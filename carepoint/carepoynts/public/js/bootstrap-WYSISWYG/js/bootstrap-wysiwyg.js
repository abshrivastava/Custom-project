			...
			updateToolbar = function () {
				if (options.activeToolbarClass) {
					$(options.toolbarSelector,wrapper).find(toolbarBtnSelector).each(underscoreThrottle(function () {
						...
						//TOMG FIX FOR line 88 and 90 to stop error with mouse move if (editor.is(':focus') && document.queryCommandState(command))
						if (commandArr.length > 1 && editor.is(':focus') && document.queryCommandEnabled(command) && document.queryCommandValue(command) == commandArr[1]) { $(this).addClass(options.activeToolbarClass);
						// Else if the command has no arguments and it is active
						} else if (commandArr.length === 1 && editor.is(':focus') && document.queryCommandEnabled(command) && document.queryCommandState(command)) {
