...
function PusherChatWidget(pusher, options) {
  ...
  this.settings = $.extend({
    ...
    replace: document.body, // A jQuery selector or object. Defines where the element should be appended to
    chatIndex:1,
    debug: true
  }, options);
  ...
  this._widget = PusherChatWidget._createHTML(this.settings.replace);
...
PusherChatWidget.prototype._chatMessageReceived = function(data) {
  ...
  messageEl.slideDown(function() {
    ...
    sessionStorage.setItem("chat"+self.settings.chatIndex+"_hx",$("#chatDialog"+self.settings.chatIndex+ " .activity-stream").html());
  });
  ...
  if(data.body=="[Chat ended]") $("#chatDialog"+self.settings.chatIndex+ " .activity-stream").css("background","#DEDEDE");
  if(!$("#chatDialog"+this.settings.chatIndex).is(":visible") ) chatGrowl(data.body,data.actor.displayName);
...
/* @private TG moified */
PusherChatWidget.prototype._closeChat = function() {
    var chatInfo = {
        text: "[Chat ended]"
};
    this._sendChatMessage(chatInfo);
    this._pusher.unsubscribe(this.settings.channelName);
};
...
/* @private */
PusherChatWidget._createHTML = function(replace) {
  var html = '' +
