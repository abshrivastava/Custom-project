...
angular.module('app.offer-detail', ['app.offer-service', 'app.member-service', 'ngMap', 'Teletubby.ui.shared.modules.nav', 'carepoynt-common-services', 'angularModalService'])

.controller('OfferDetailCtrl', ['$scope', '$q', '$filter', '$sce', '$state', '$stateParams', '$window', 'memberService', 'ModalService', 'offerService', 'featureToggleService', 'NgMap', 'TeletubbyNav', 'ezfb','alertService',
            function($scope, $q, $filter, $sce, $state, $stateParams, $window, memberService, ModalService, offerService, featureToggleService, NgMap, TeletubbyNav, ezfb, alertService) {
    const vm = $scope;
    vm.goTo = goTo;
    vm.goBack = goBack;
    vm.isEarn = isEarn;
    ...
    // ******************************
      ...
      .then(function (response) {
        ...
        if(response && response.purchaseShowInfo){
          ...
          alertService.error("Error while fetching purchase warning modal user preference. Please try again.");
        }
        ...
        if(vm.offer.promo_type == 'refer_business' && vm.offer.user_earned == false){
          vm.showEarnButton = true;
          vm.offer.action.url = 'refer-business';
        }
  ...
  function getCheckin() {
    ...
    function earnClick(){
      ...
      if(vm.offer.promo_type == 'review_social') {
        ...
        //             }
        //             if (authorized) {
        //               console.log("you have the proper permissions :)");
        //               vm.showFacebookCheck = true;
        //               vm.socialChecks.Facebook = true;
        //             }
        //             else {
    ...
    function redeemClick(){
      ...
      offerService.redeemOffer(vm.offer)
        .then(function (response) {
          ...
          vm.showBusy = false;
          alertService.error("Error while saving voucher details. Please try again.");
        });
    }
        ...
        offerService.promoEarned(vm.offer)
          .then(function (response) {
            if(response.success) {
              ...
              vm.enoughPoynts = hasEnoughtPoynts();
              vm.userEarned = true;
              if(poyntsEarned && poyntsEarned > 0)
              {
                alertService.success("You earned " + poyntsEarned + " poynts for watch the video promotion.");
              }
            }
          });
      }
          ...
          offerService.promoSocialRating(vm.offer)
            .then(function (response) {
              if(response.success) {
                ...
                .then(function (response) {
                  if(response.success) {
                    vm.showSocialReviewScreen = false;
                    vm.showReviewDone = true;
                    vm.showBusy = false;
                    // alertService.success("You earned " + response.totalPoynts + " poynts for submitting a review.");
                  }
                }); 
              }
            });
    ...
    function submitReferral() {
      ...
      vm.offer.referral = vm.referral;
      if(!vm.userEarned) {
        offerService.promoReferral(vm.offer)
          .then(function (response) {
            
            if(response.msg == 'Already a member.') {
              vm.showMemberAlreadyExists = true;
            }
            else if(response.msg == 'Already invited.'){
              vm.showMemberAlreadyInvited = true;
            }
              ...
              canEarn();
              vm.showBusy = false;
              vm.showMemberInvited = true; 
              if(poyntsEarned && poyntsEarned > 0)
              {
                alertService.success("You earned " + poyntsEarned + " poynts for submitting a referal.");
              }
            }
    ...
    */
    ...
    function showWarningModal(){
      window.affiliateUrl = vm.offer.action.url, '_affiliate_' + vm.offer.id;

      ModalService.showModal({
        templateUrl: "offer-detail/affiliate-warning-model.html",
      ...
      .then(function(modal) {
        ...
        modal.close.then(function(data) {
          if(data.result) {
            // $window.open(vm.offer.action.url, '_affiliate_' + vm.offer.id); 
            if(data.check) {
              offerService.setAffiliateShowAgainTag()
                .then(function(response){
                    ...
                    alertService.error("Unable to update user preference at service. Please try again.");
                })
            }
          }
    ...
    function showWarningModalForPurchase(){
      ModalService.showModal({
        templateUrl: "offer-detail/purchase-warning-model.html",
        controller: "ModalController"
      })
      .then(function(modal) {
        modal.element.modal({
        ...
        modal.close.then(function(data) {
          if(data.result) {
            if(data.check) {
              offerService.setPurchaseShowAgainTag()
                .then(function(response){
                    if(response && response.success){
