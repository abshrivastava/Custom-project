...
.controller('DosGetsCtrl', ['$scope', '$q', '$state', '$stateParams', '$window', 'onboardService', function($scope, $q, $state, $stateParams, $window, onboardService) {
    ...
    function trackGoogleAnalytics() {
      if(vm.step == 0) {
        ga('set', 'page', "DosGets Onboard Profile");
        ga('send', 'pageview');
      }
      if(vm.step == 1) {
        ga('set', 'page', "DosGets Onboard Referral");
        ga('send', 'pageview');
      }
      if(vm.step == 2) {
        ga('set', 'page', "DosGets Onboard Clique");
        ga('send', 'pageview');
      }
      if(vm.step == 3) {
        ga('set', 'page', "DosGets Onboard Success");
        ga('send', 'pageview');
      }     
      if(vm.step == 4) {
        ga('set', 'page', "DosGets Onboard Confirmation");
        ga('send', 'pageview');
      } 
      if(vm.step == 5) {
        ga('set', 'page', "DosGets Onboard Error");
        ga('send', 'pageview');
      }        
    }
