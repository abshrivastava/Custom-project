...
angular.module('app.dosgets3ptnr',[])

.controller('DosGetsCtrl3ptnr', ['$scope', '$q', '$state', '$stateParams', '$window', 'onboardService', function($scope, $q, $state, $stateParams, $window, onboardService) {
    const vm = $scope;
    ...
    vm.goDosgetsValidateAndMakeRoll = goDosgetsValidateAndMakeRoll;
    vm.goMakeRoll = goMakeRoll;
    ...
    function init() {
      if(getCookie("dosgets") == '') {
        onboardService.goDosgetsData($stateParams.name)
        ...
        vm.config.msg = vm.config.msg.replace(/\+/g, ' ');
        initParams();     
      }     
    }
    ...
    function initParams() {
      ...
      vm.phoneReadOnly = false;

      if(getCookie('onboard_email')) {
        vm.profile.email = decodeURIComponent(getCookie('onboard_email'));
        vm.emailReadOnly = true;
        vm.profile.emailR = true;
      }

      if(getCookie('onboard_phone')) {
        vm.profile.mPhone = decodeURIComponent(getCookie('onboard_phone'));
        vm.phoneReadOnly = true;
        vm.profile.phoneR = true;
      }
    ...
    function goDosgets() {
      ...
      vm.dosgets.path = $stateParams.name;
    ...
    function goMakeMe(step) {
      ...
      if(vm.emailReadOnly || vm.phoneReadOnly) {
    ...
    function checkEmail() {
      vm.emailError = "";
      if(!vm.emailReadOnly) {
        if(vm.profile.email != null && vm.profile.email != '') {
          onboardService.checkEmail(encodeURIComponent(vm.profile.email))
            .then(function(response) {
              if(response.success) {
                vm.form.profileForm.$setValidity('cpEmail', false);
                vm.emailError = "That email is taken. Try another."
                //email in use
              }
              else {
                vm.form.profileForm.$setValidity('cpEmail', true);
              }
            });
        }
        else {
          vm.form.profileForm.$setValidity('cpEmail', false);
          vm.emailError = "Please put in an email address.";
        }
      }
    }
    ...
    function checkPhone() {
      vm.phoneError = "";
      if(!vm.phoneReadOnly) {
        if(vm.profile.mPhone != '' && vm.profile.mPhone != null) {
          onboardService.checkPhone(encodeURIComponent(vm.profile.mPhone))
            .then(function(response) {
              if(response.success) { 
                //phone in use
                vm.form.profileForm.$setValidity('mobilePhone', false);
                vm.phoneError = "That phone number is taken. Try another."
              }
              else {
                vm.form.profileForm.$setValidity('mobilePhone', true);
              }
            });
        }
        else {
          vm.form.profileForm.$setValidity('mobilePhone', false);
          vm.phoneError = "Please put in a phone number.";
        }
      }
    }
    ...
    function goDosgetsValidateAndMakeRoll() {
      let dob = (new Date(vm.profile.dob));
      if(vm.profile.dob && vm.profile.dob != "") {
        vm.profile.dobDisplay = dob.getMonth()+1+'/'+dob.getDate()+'/'+dob.getFullYear();
      }
    ...
    function goMakeRoll() {
      ...
      if(vm.emailReadOnly) {
        onboardService.goDosgetsValidate(vm.dosgets)
          .then(function(response) {
            if(response.success) {
              ...
              onboardService.goDosgetsRollUpdate(vm.dosgets)
                .then(function(response) {
                  console.log('done');
                });
              goToStep(3);
        ...
        onboardService.goDosgetsValidate(vm.dosgets)
          .then(function(response) {
            if(response.success) {
              ...
              onboardService.goDosgetsRoll(vm.dosgets)
                .then(function(response) {
                  console.log('done')
                });
              goToStep(3);
        ...
        // onboardService.goMakeRollMe(vm.dosgets)
        //   .then(function(response) {
        //     if(response.success) {
        //       vm.step = step;
        //       trackGoogleAnalytics();
