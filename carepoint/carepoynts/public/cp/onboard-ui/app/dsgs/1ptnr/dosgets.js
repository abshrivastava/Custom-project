...
angular.module('app.dosgets1ptnr',[])

.controller('DosGetsCtrl1ptnr', ['$scope', '$q', '$state', '$stateParams', '$window', 'onboardService', function($scope, $q, $state, $stateParams, $window, onboardService) {
    const vm = $scope;
    ...
    function goDosgets() {
      ...
      onboardService.goDosgetsValidate1Step(vm.dosgets)
        .then(function(response) {
          vm.results = response;
          if(response.success) {
            if(vm.results.profile.success) {
              vm.gotPromo = true;    
            }
            vm.showBusy = false;
            goToStep(1);
            if(vm.emailReadOnly || vm.phoneReadOnly) {//do
              onboardService.goDosgets1StepUpdate(vm.dosgets)
                .then(function (response) {
                  console.log("done");
                });              
            }
            else {//update
              onboardService.goDosgets1Step(vm.dosgets)
                .then(function (response) {
                  console.log("done");
                });
            }
    ...
    function goMakeMe(step) {
      ...
      onboardService.goMakeMe1Step(vm.dosgets)
        .then(function(response) {
          if(response.success) {
            vm.step = step;
            trackGoogleAnalytics();
