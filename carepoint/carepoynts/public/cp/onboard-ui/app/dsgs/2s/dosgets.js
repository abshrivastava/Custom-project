...
angular.module('app.dosgets2', [])

.controller('DosGetsCtrl2', ['$scope', '$q', '$state', '$stateParams', '$window', 'onboardService', function($scope, $q, $state, $stateParams, $window, onboardService) {
    const vm = $scope;
    ...
    function goDosgets() {
      ...
      onboardService.goDosgetsValidate2Step(vm.dosgets)
        .then(function(response) {
          if(response.success) {
            vm.results = response;
            if(vm.results.profile.success && vm.results.referral.success) {
              vm.gotPromo = true;
              
            }

            onboardService.goDosgets2Step(vm.dosgets)
              .then(function(response) {
                console.log('done');
              });
            goToStep(2);
